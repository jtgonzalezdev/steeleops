...
def application(environ, start_response):
    ...
    if path == '/time-off/approve' and method == 'POST':
        ...
        if user['role'] == 'supervisor':
            assigned_shift = conn.execute(
                'SELECT site_id FROM shifts WHERE company_id=? AND COALESCE(user_id, guard_id)=? ORDER BY shift_date DESC LIMIT 1',
                (user['company_id'], req['guard_id']),
            ).fetchone()
            if not assigned_shift or not supervisor_can_access_site(conn, user, assigned_shift['site_id']):
