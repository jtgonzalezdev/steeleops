...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS manual_clock_in_requests (
        id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        company_id INTEGER NOT NULL,
        guard_id INTEGER NOT NULL,
        site_id INTEGER NOT NULL,
...
def request_manual_clock_in(user, reason=None, comments=None):
    ...
    try:
        ...
        conn.close()
    return True, 'manual clock-in requested'
...
def review_manual_clock_in_request(user, request_id, decision):
    ...
    conn = db()
    try:
        req = conn.execute('SELECT * FROM manual_clock_in_requests WHERE id=? AND company_id=?', (request_id, user['company_id'])).fetchone()
        if not req:
    ...
    return {
        ...
        **patrol_data,
        ...
        'supervisor_dashboard_summary': {
            ...
            'pending_approvals': len([r for r in time_corrections if (r.get('status') or '').lower() == 'pending']) + len([r for r in manual_clock_in_requests if (r.get('status') or '').lower() == 'pending']) + len([r for r in admin_time_off_requests if (r.get('status') or '').lower() == 'pending']),
...
def application(environ, start_response):
    ...
    if path == '/manual-clock-in/approve' and method == 'POST':
        user, response = require_admin(environ, start_response)
        if response: return response
        data, _ = parse_post(environ); ok, message, target_shift_id = review_manual_clock_in_request(user, data.get('request_id'), data.get('decision'))
        if not ok: return bad_request(start_response, message)
        log_audit('manual_clock_in_review', actor_user_id=user['id'], company_id=user['company_id'], target_type='shift', target_id=target_shift_id or data.get('request_id'), message=message, environ=environ); return redirect(start_response, '/dashboard')
...
def application(environ, start_response):
    ...
    if path == '/manual-clock-in/approve' and method == 'POST':
        ...
        if response: return response
        data, _ = parse_post(environ)
        ok, message, target_shift_id = review_manual_clock_in_request(user, data.get('request_id'), data.get('decision'))
        if not ok:
            return bad_request(start_response, message)
