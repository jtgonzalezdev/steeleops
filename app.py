...
def init_db():
    ...
    if table_exists(conn, 'clients'):
        for col in ['company_id INTEGER', 'active INTEGER DEFAULT 1', 'created_at TEXT']:
    ...
    # backfill clients from existing site text field (safe, idempotent)
    site_clients = conn.execute("""
        SELECT DISTINCT company_id, client_company_name
        FROM sites
        WHERE COALESCE(client_company_name, '') != ''
    """).fetchall()
    for row in site_clients:
        existing_client = conn.execute(
            'SELECT id FROM clients WHERE company_id=? AND name=?',
            (row['company_id'], row['client_company_name'])
        ).fetchone()
        if existing_client:
            client_id = existing_client['id']
        else:
            conn.execute(
                'INSERT INTO clients (company_id, name, active, created_at) VALUES (?, ?, 1, ?)',
                (row['company_id'], row['client_company_name'], now)
            )
        ...
        conn.execute(
            'UPDATE sites SET client_id=? WHERE company_id=? AND client_company_name=? AND client_id IS NULL',
            (client_id, row['company_id'], row['client_company_name'])
        )

    site_rows = conn.execute('SELECT id, client_id, client_company_name FROM sites WHERE client_id IS NOT NULL').fetchall()
    for row in site_rows:
        client = conn.execute('SELECT name FROM clients WHERE id=?', (row['client_id'],)).fetchone()
        if client and (row['client_company_name'] is None or row['client_company_name'] == ''):
            conn.execute('UPDATE sites SET client_company_name=? WHERE id=?', (client['name'], row['id']))
...
def application(environ, start_response):
    ...
    if path == '/admin/site/new' and method == 'POST':
        ...
        if response:
            return response
        data, _ = parse_post(environ)
        client_name = (data.get('client_company_name') or '').strip()
        ...
        if client_id:
            ...
            existing = conn.execute('SELECT id FROM clients WHERE company_id=? AND name=?', (user['company_id'], client_name)).fetchone()
            if existing:
                client_id = existing['id']
            else:
                conn.execute('INSERT INTO clients (company_id, name, active, created_at) VALUES (?, ?, 1, ?)', (user['company_id'], client_name, datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
                client_id = conn.execute('SELECT id FROM clients WHERE company_id=? AND name=?', (user['company_id'], client_name)).fetchone()['id']
        conn.execute('''INSERT INTO sites (company_id, client_id, name, client_company_name, address, notes, active) VALUES (?, ?, ?, ?, ?, ?, 1)''',
                     (user['company_id'], client_id, data.get('name'), client_name, data.get('address'), data.get('notes')))
        conn.commit(); conn.close()
        return redirect(start_response, '/dashboard')
    ...
    if path == '/admin/client/new' and method == 'POST':
        ...
        if not name:
            return redirect(start_response, '/dashboard')
        conn = db()
        existing = conn.execute('SELECT id FROM clients WHERE company_id=? AND name=?', (user['company_id'], name)).fetchone()
        if not existing:
            conn.execute('INSERT INTO clients (company_id, name, active, created_at) VALUES (?, ?, 1, ?)', (user['company_id'], name, datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
            conn.commit()
        conn.close()
        return redirect(start_response, '/dashboard')
    ...
    <section class="grid two-col">
      <div class="card">
        ...
        <form method="post" action="/admin/client/new" class="stack compact">
          ...
          <label>Client Name<input type="text" name="name" required></label>
          <button class="btn" type="submit">Add Client</button>
        </form>
        <hr>
        <form method="post" action="/admin/site/new" class="stack compact">
          <h4>Sites Module</h4>
          <div class="row-2"><label>Site Name<input type="text" name="name" required></label><label>Client<select name="client_id"><option value="">— Select Client —</option>{% for client in clients %}<option value="{{ client.id }}">{{ client.name }}</option>{% endfor %}</select></label></div>
          <label>Client Company Name (optional fallback)<input type="text" name="client_company_name"></label>
    ...
    if path == '/admin/site/new' and method == 'POST':
        ...
        if client_id:
            ...
            if existing:
                ...
                conn.execute('INSERT INTO clients (company_id, name, active, created_at) VALUES (?, ?, 1, ?)', (user['company_id'], client_name, datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
                client_id = conn.execute('SELECT id FROM clients WHERE company_id=? AND name=?', (user['company_id'], client_name)).fetchone()['id']
        conn.execute('INSERT INTO sites (company_id, client_id, name, client_company_name, address, notes, active) VALUES (?, ?, ?, ?, ?, ?, 1)', (user['company_id'], client_id, data.get('name'), client_name, data.get('address'), data.get('notes')))
        conn.commit(); conn.close(); log_audit('admin_action', actor_user_id=user['id'], company_id=user['company_id'], target_type='site', target_id=data.get('name'), message='site created', environ=environ); return redirect(start_response, '/dashboard')
    if path == '/admin/client/new' and method == 'POST':
        user, response = require_admin(environ, start_response)
        if response: return response
        data, _ = parse_post(environ)
        ...
        if not existing:
            conn.execute('INSERT INTO clients (company_id, name, active, created_at) VALUES (?, ?, 1, ?)', (user['company_id'], name, datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
            conn.commit()
        conn.close(); log_audit('admin_action', actor_user_id=user['id'], company_id=user['company_id'], target_type='client', target_id=name, message='client created', environ=environ); return redirect(start_response, '/dashboard')
    if path == '/admin/shift/new' and method == 'POST':
        user, response = require_admin(environ, start_response)
        if response: return response
